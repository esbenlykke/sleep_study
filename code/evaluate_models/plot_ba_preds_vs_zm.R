...
# TODO Fix theming for journal publication
...
# diff_avg <-
  stats %>% 
    mutate(
    diff_spt_hrs = spt_hrs - zm_spt,
    diff_tst_hrs = tst_hrs - zm_tst,
    diff_se_percent = se_percent - zm_se,
    diff_lps_min = lps_min - zm_lps,
    diff_waso_min = waso_min - zm_waso
  ) |>
    rowwise() |>
    mutate(
      avg_spt_hrs = mean(c(spt_hrs, zm_spt)),
      avg_tst_hrs = mean(c(tst_hrs, zm_tst)),
      avg_se_percent = mean(c(se_percent, zm_se)),
      avg_lps_min = mean(c(lps_min, zm_lps)),
      avg_waso_min = mean(c(waso_min, zm_waso))
    )
